...
WITH base AS (
  SELECT
    ...
    poit.next_expected_arrival AS next_expected_arrival,
...
po_due_lt AS (
  SELECT
    b.product_id,
    COALESCE(
      SUM(po.qty_ordered) FILTER (
        WHERE po.status = 'ORDERED'
          AND po.expected_arrival IS NOT NULL
          AND po.expected_arrival <= (CURRENT_DATE + GREATEST(b.lead_time_days, 0)::int)
      ),
      0
    )::int AS in_transit_due_lt
  FROM base b
  LEFT JOIN purchase_orders po ON po.product_id = b.product_id
  GROUP BY b.product_id, b.lead_time_days
),
fc AS (
...
calc AS (
  SELECT
    ...
    COALESCE(pod.in_transit_due_lt, 0) AS in_transit_due_lt,
    CEIL(COALESCE(fc.forecast_daily_sales, 0) * b.lead_time_days + b.safety_stock_units) AS rop_base
  FROM base b
  LEFT JOIN po_due_lt pod ON pod.product_id = b.product_id
...
SELECT
  ...
  CASE
    -- Status considers inbound that is expected within the lead-time window:
    -- If (available + in_transit_due_lt) is still <= ROP => CRITICAL, else SAFE.
    WHEN (c.available + COALESCE(c.in_transit_due_lt, 0)) <= COALESCE(c.override_rop_units, c.rop_base)
...
SELECT
  ...
  CASE
    ...
    ELSE (s.stock_for_risk - x.rop_units) / x.forecast_daily_sales
  END AS days_until_rop,

  CASE
    WHEN x.forecast_daily_sales <= 0 THEN FALSE
    WHEN (s.stock_for_risk - x.rop_units) / x.forecast_daily_sales <= cfg.risk_horizon_days THEN TRUE
    ELSE FALSE
  END AS at_risk_60d
...
-- Only count inbound POs that are expected BEFORE we reach ROP (based on the current burn rate).
CROSS JOIN LATERAL (
  SELECT
    CASE
      WHEN x.forecast_daily_sales <= 0 THEN x.available
      ELSE (
        x.available
        + COALESCE((
            SELECT SUM(po.qty_ordered)::int
            FROM purchase_orders po
            WHERE po.product_id = x.product_id
              AND po.status = 'ORDERED'
              AND po.expected_arrival IS NOT NULL
              AND po.expected_arrival <= (
                CURRENT_DATE
                + GREATEST(CEIL((x.available - x.rop_units) / x.forecast_daily_sales)::int, 0)
              )
        ), 0)
      )
    END AS stock_for_risk
) s;
  ...
  SELECT
    ...
    COALESCE(
      SUM(po.qty_ordered) FILTER (
        WHERE po.status = 'ORDERED'
          AND po.expected_arrival IS NOT NULL
          AND po.expected_arrival <= (CURRENT_DATE + cfg.reorder_coverage_days)
      ),
      0
    )::int AS in_transit_due_cov
  FROM products p
  CROSS JOIN cfg
  LEFT JOIN purchase_orders po ON po.product_id = p.id
  GROUP BY p.id, cfg.reorder_coverage_days
),
base AS (
  SELECT
    d.*,
    CEIL(d.forecast_daily_sales * cfg.reorder_coverage_days + d.safety_stock_units) AS target_units_cov,
    (d.available + COALESCE(pc.in_transit_due_cov, 0)) AS effective_stock
  FROM v_logistics_daily_ext d
  CROSS JOIN cfg
  LEFT JOIN po_cov pc ON pc.product_id = d.product_id
),
need AS (
  SELECT
    b.*,
